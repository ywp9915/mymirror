 ...
 */
let validate = (DataSetName, Validator, InputValue, context) => {
  let form = context.data.form || {}
  let name = DataSetName
  let value = (InputValue || '').trim()
  let validator = Validator ? Validator.split(',') : []
  if (name) {
    ...
    form.$invalidMsg = ''
    for (let p in form) {
      if (p !== '$invalidMsg' && p !== '$dirty') {
  
        form.$invalidMsg = form[p].trim() ? form[p] : form.$invalidMsg
      }
    }
    context.setData({
      form
